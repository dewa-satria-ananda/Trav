    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-6 mid border-right border-left pt-4 px-0">
                <div class="card card-border-radius card-shadow post-card mb-3">
                    <div class="row border-bottom" style="height: 52px;">
                        <div class="col-8">
                        ...
                        @if ($User->userId != auth()->user()->userId)
                                ...
                                <div class="d-flex flex-row-reverse mr-4 mt-2">
                                    @if ($Follow)
                                        <form action="{{ route('follow', $User->userId) }}" method="POST"
                                            ...
                                            {{ csrf_field() }}
                                            <button class="like btn cancelBtn rounded-pill " type="submit" style="">
                                                Unfollow</button>
                                        </form>
                                    @else
                                        <form action="{{ route('follow', $User->userId) }}" method="POST"
                                            enctype="multipart/form-data">
                                            {{ csrf_field() }}
                                            <button class="like btn rounded-pill add-post addPost-button" type="submit"
                                                style=""> Follow</button>
                                        </form>
                                    @endif
                                </div>
                            ...
                            <div class="col-2">
                                <div class="d-flex flex-row-reverse ml-3 mt-2">
                                    <form action="{{ route('start-chat', $User->userId) }}" method="POST"
                                        enctype="multipart/form-data">
                                        {{ csrf_field() }}
                                        <button class="like btn rounded-pill add-post addPost-button" type="submit"
                            ...
                            <div class="row pb-4">
                                <div class="col-2">
                                    <img class="post-account-img rounded-circle"
                                        src="{{ asset('storage/profileImage/' . $User->image) }}" alt="">
                                </div>
                                <div class="col-10 pl-4 pt-1">
                                    <p class=" post-name mt-4">{{ $User->name }}</p>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-6">
                                    <p class="profile-subtitle mb-0">{{ $User->followers }}</p>
                                    <p class="profile-title">followers</p>
                                </div>
                                <div class="col-6">
                                    <p class="profile-subtitle mb-0">{{ $User->following }}</p>
                                    <p class="profile-title">following</p>
                                </div>
                            </div>
                            <h5 class="profile-title">Bio</h5>
                            <p class="profile-subtitle">{{ $User->bio }}</p>
                        </div>
                    </div>
                </div>
                <div class="post border-bottom pb-0 mb-3">
